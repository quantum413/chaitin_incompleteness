...
mod tests {
    ...
    // #[test]
    // fn parens_parsing() {
    //     assert!(PsParser::parse("(").is_none());
    //     assert!(PsParser::parse("").unwrap().get_tokens)
    // }
    fn parse_and_extract(rule: Rule, str: &str) -> Pair<Rule>{
        FOL::parse(rule, str).unwrap().next().unwrap()
    }
    ...
    fn symbols_parsing() {
        ...
        assert!(FOL::parse(Rule::formula, &["(@ ", non_var_term, " ", str_3_2, ")"].concat()).is_err());
    ...
    #[test]
    fn variable_parsing() {
        let parse_1 = FOL::parse(Rule::just_variable, "vxyz");
        ...
        assert!(FOL::parse(Rule::just_variable, "zzz").is_err());
        assert!(FOL::parse(Rule::just_variable, "v*").is_err());
    }
    ...
    fn pred_parsing(){
        ...
        check_pair_eq(iter_2.next().unwrap(), FOL::parse(Rule::term, "v0").unwrap().next().unwrap());
        check_pair_eq(iter_2.next().unwrap(), FOL::parse(Rule::term, "(f__A v1 v2)").unwrap().next().unwrap());
        assert_last(iter_2);

        check_parse_eq(FOL::parse(Rule::formula, str_1), FOL::parse(Rule::predicate, str_1));
        check_parse_eq(FOL::parse(Rule::formula, str_2), FOL::parse(Rule::predicate, str_2));
    }

    fn parse_and_check_top(str: &str, parse_rule: Rule, expected: Rule) -> Pairs<Rule> {
        check_top(str, FOL::parse(parse_rule, str).unwrap(), expected)
    }
    ...
    #[test]
    fn term_parsing() {
        assert!(FOL::parse(Rule::function, "(fa vx)").is_err());
        let str_1 = "(fab)";
        let var_str = "vxy";
        let str_2 = "(f_ab vxy)";
        ...
        assert!(FOL::parse(Rule::function, "(f__abc vxy)").is_err());
        assert!(FOL::parse(Rule::function, "(f_abc vxy vzw)").is_err());

        check_parse_eq(FOL::parse(Rule::variable, var_str), FOL::parse(Rule::term, var_str));
        check_parse_eq(FOL::parse(Rule::function, str_1), FOL::parse(Rule::term, str_1));
        check_parse_eq(FOL::parse(Rule::function, str_2), FOL::parse(Rule::term, str_2));
        check_parse_eq(FOL::parse(Rule::function, str_3), FOL::parse(Rule::term, str_3));
        ...
        check_pair_eq(iter_4.next().unwrap(), FOL::parse(Rule::term, "(f_a v1)").unwrap().next().unwrap());
        check_pair_eq(iter_4.next().unwrap(), FOL::parse(Rule::term, "(f__ab v2 (f))").unwrap().next().unwrap());
        assert_last(iter_4);
    }
    ...
    fn zero_ary_functions() {
        let parse_0 = FOL::parse(Rule::function, "fa");
        assert!(parse_0.is_err());
        let parse_1_wrapped = FOL::parse(Rule::function, "(fa)");
